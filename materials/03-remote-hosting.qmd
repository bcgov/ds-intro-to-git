...
Let's create a file called `a.csv` that contains some data about the Red Planet's average temperature. Open a new csv named `a.csv` using Excel and add two columns named 'Time' and 'Temperature'. Add a few rows of data if you want and close the file. It doesn't really matter what data is in the file, only that it is in our directory, planets. Type the ls command to be certain, then check the output of git status.
...
We can also ignore full subfolders by adding the folder pathname to our .gitignore. Add another line to your .gitignore file so it looks like:
...
And then add a .txt file to that subfolder. Again, open up your favorite text editor and add some text to a file, then save your changes. Switch back to command line when you're finished and check the git status.
...
The subfolder, results, and it's contents are ignored. This is handy for keeping all of your data in one place.

...
You configured Git by setting up your username and email address, which ensures that every change you commit is associated with your identity. Most of you did this in Git Bash terminal. If you need a refresher, the configuration steps can be found here. (next)

You set up a GitHub account, which will allow you to store and collaborate on code projects in a cloud-based repository. (next) As part of this process you enabled Two-Factor Authentication (2MFA) to enhance account security. (next) You also created a Personal Access Token to allow Git to authenticate securely with your GitHub account for operations like pushing or pulling code. (next)
...
Question: Do we want to check configuration at some point?
...
Version control is the practice of tracking and managing changes to software code. Version control systems (like Git) are software tools that are used to help teams manage changes to code/documents over time.
...
In this diagram person green takes a copy and person red also takes a copy.  They add some lines and when they are finished making changes to their own copy...
...
Unless multiple users make changes to the same section of the document (a conflict) you can incorporate two sets of changes into the same base document.  (next)
...
At the top level, we have the wes-analysis folder, which is the root directory of this repository. This is where all our files and folders for the project are organized.  Inside this folder you likely have other project-related subfolders such as: 
 ...
 - **doc** and **ref** containin files such as a list of acronyms, reports, reference materials and possibly articles and images for visual documentation or presentations. This is very much project-dependent. 
...
- **data** contains the raw data files in this hypothetical project.  It’s worth noting that sensitive or private data, like these CSV files, won’t become public as we will give explicit instructions to git to ignore them. (next)
...
The **.gitignore** *file* lists files or patterns we don’t want Git to track—like large data files such as the CSVs. This is a common practice to keep repositories clean and focused on the essential project elements. (next)
...
Git is a distributed version control system designed to track changes in files, (next)particularly source code. (next)
...
Git supports parallel development, allowing teams to work on different features or fixes without interference.  This is done through Branching and Merging which we will review shortly. (next)

Git is a distributed Model, meaning every user has a full copy of the repository.  The benefit here is that operations require only local files and resources.  (This makes Git extremely fast and reliable). (next)
...
Before we move into live coding, let's preview a few Bash commands.  The commands in the Basic column are ones that we'll use in the workshop.
...
**cd path/to/directory**: This command lets you navigate to a specific directory. Think of it as ‘changing your location’ in the file system.  (next)
   - If the directory path includes spaces, make sure to enclose it in quotes, like `cd "My Folder"`.  (next)
...
**`ls -a`** Adding the `-a` flag shows all files, including hidden ones. Hidden files usually start with a dot (`.`), like `.git`, and they’re essential for tasks like managing Git repositories.  (next)
...
**`git init`**: This command initializes a Git repository in the current directory. It creates a hidden `.git` folder, which Git uses to track changes to your files. You only need to do this once per project to start version control.  (next)

**`git status`**: Running this command gives you a snapshot of your repository’s current state. It will show any changes to files, whether they’ve been staged for commit, and if there are untracked files. It’s like your ‘to-do list’ for what’s happening in Git.  (next) 

**`git add <filename>`** : This stages a file or changes to a file, adding it to the next commit. Replace `<filename>` with the name of the file you want to add. If you want to stage all changes at once, you can use `git add .`. (next)
...
**`git log`**: Use this command to view the commit history of your repository. It shows who made the changes, when they were made, and the commit messages. It’s an excellent way to review the progress of your project.  (next)

**`git diff`**:  This displays the changes between commits or the differences between your working directory and the last commit. It’s useful for checking exactly what has changed before committing.  (next)
   
**`git restore`**:  This restores files to a previous commit, effectively undoing changes. It’s handy if you want to discard changes or return a file to a specific version.  (next)
...
- **git add \<filename>** and **git commit -m 'message'**
